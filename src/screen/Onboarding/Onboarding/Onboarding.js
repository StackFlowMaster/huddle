...
const Onboarding = ({ navigation }) => {
  ...
  const { useBioAuth, accessToken } = useSelector(selectors.onboardingSelector);
  ...
  useEffect(() => {
        ...
        if (useBioAuth) {
          ...
          try {
            ...
            if (!offlineMode.isOffline) {
              await dispatch(actions.login(pinCode));
            }
    ...
    if (!userHasBeenDirected.current) {
      dispatch(biometricAuthActions.checkBiometricSupport());
      directUser();
    }
  }, [imageSize]);
  ...
  const handleStartPress = () => {
    dispatch(mixpanelActions.trackEvent(loginEvents.CLICK_GET_STARTED));
