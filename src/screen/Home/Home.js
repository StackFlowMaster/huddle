...
import { actions as activityActions, selectors as activitySelectors } from '/state/activity';
...
const Home = ({ navigation }) => {
  ...
  const activity = useSelector(activitySelectors.activitySelector) || [];
  ...
  const profileCode = get(activeProfile, 'profileCode');
  ...
  useEffect(() => {
    if (profileCode) {
      dispatch(itemsActions.fetchItems(false, activeProfile.profileCode));
      dispatch(activityActions.fetchActivity(activeProfile.profileCode));
    }
  }, [profileCode]);
  ...
  const handleScanInsurancePress = () => {
    ...
    navigation.push(screens.ScanInsuranceCardStack);
  };
